...
func main() {
	...
	go binance.CheckSell("BTC", []string{"PostBankNew", "RussianStandardBank"}, "http://user129953:97w4lt@45.135.248.216:2045")
	assets := []string{"BTC"}
	current_proxy_num := 0
	for {
		for _, asset := range assets {
			...
			go binance.CheckAsset(user_min_money, user_max_money, need_spread, asset, []string{"PostBankNew", "RussianStandardBank"}, proxies[current_proxy_num])
		}
